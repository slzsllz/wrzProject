...
def safe_open(*args, **kwargs):
    if len(args) > 0 and isinstance(args[0], str):
        filename = args[0]
        if filename in ["使用须知.txt", "环境配置.txt"] or filename.endswith("使用须知.txt") or filename.endswith("环境配置.txt"):
            from io import StringIO
            return StringIO()
    return builtins.open(*args, **kwargs)  # 使用builtins.open代替_original_open

builtins.open = safe_open

# 检查并删除作者相关文件
def check_and_delete_author_files():
...
if __name__ == '__main__':
    img_path = "test_/plastic173_jpg.rf.bcdde112397eddf4497054efb702999b.jpg"
